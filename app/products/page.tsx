      ...
      <div className="container mx-auto px-4">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {categories.map((category, index) => (
            <SectionReveal as="div" delay={index * 150} key={category.id}>
              <Link
                href={`/products/${category.id}`}
                className="group glass-card rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 block h-full"
              >
                {/* Image */}
